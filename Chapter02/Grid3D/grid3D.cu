...
{
	int x = blockIdx.x * blockDim.x + threadIdx.x; // find
	int y = blockIdx.y * blockDim.y + threadIdx.y; // (x,y,z)
...
int main(int argc, char *argv[])
{
	...
	cudaDeviceSynchronize(); // neccesary in Linux to see kernel printf
	return 0;
}